...
import { useGetCandidateProfile, useListAssessments } from "@workspace/api-client-react";
import { AnimatedPage, Card, CardContent, Badge, Button } from "@/components/ui-elements";
import { Clock, HelpCircle, Code2, Brain, Wrench, Trophy, ArrowRight } from "lucide-react";
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/10 flex flex-col items-center justify-center min-w-[160px]">
            ...
            <span className="text-3xl font-bold font-mono">
              {profile?.overallScore !== undefined && profile?.overallScore !== null ? `${profile.overallScore}%` : 'N/A'}
            </span>
          </div>
        </div>
      </div>
          ...
          >
            <Card className="h-full flex flex-col hover:-translate-y-1 transition-transform duration-300 border-border/50">
              <CardContent className="p-6 flex flex-col flex-grow">
                ...
                <h3 className="text-xl font-bold font-display mb-2">{assessment.title}</h3>
