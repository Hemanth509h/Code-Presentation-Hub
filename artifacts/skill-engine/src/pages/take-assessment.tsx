...
import { AnimatedPage, Button, Card, CardContent } from "@/components/ui-elements";
        ...
        <Card className="max-w-md w-full text-center shadow-2xl">
          <CardContent className="pt-10 pb-8 flex flex-col items-center">
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 ${resultData.passed ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
            >
              <CheckCircle2 className="w-12 h-12" />
            </motion.div>
            <h2 className="text-3xl font-display font-bold mb-2">Assessment Complete</h2>
            <p className="text-muted-foreground mb-8 text-lg">
              You scored <span className="font-bold text-foreground">{resultData.score} / {resultData.maxScore}</span> ({resultData.percentage}%)
            </p>
            <div className={`p-4 rounded-xl mb-8 w-full ${resultData.passed ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
              {resultData.feedback}
            </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 w-full flex-grow flex flex-col">
      ...
      <div className="flex items-center justify-between mb-8 bg-card border shadow-sm rounded-2xl p-4">
        <div>
          <h2 className="font-display font-bold text-lg">{assessment.title}</h2>
          <p className="text-sm text-muted-foreground">Question {currentIndex + 1} of {assessment.questions.length}</p>
        </div>
              ...
              {question.type === 'multiple_choice' && question.options && (
                <div className="space-y-4 mt-8">
                  {question.options.map((opt, idx) => {
                    const isSelected = answers[question.questionId]?.selectedOption === idx;
                    return (
                      <button
                        key={idx}
