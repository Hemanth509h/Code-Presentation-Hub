...
import { useGetCandidateRankings, useGetRecruitmentStats, GetCandidateRankingsAssessmentType } from "@workspace/api-client-react";
import { AnimatedPage, Card, CardContent, Badge } from "@/components/ui-elements";
...
export default function RecruiterDashboard() {
  const [filterType, setFilterType] = useState<GetCandidateRankingsAssessmentType>("all");
    ...
    <AnimatedPage className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <Card>
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold">{stats?.totalCandidates || 0}</h4>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold">{stats?.assessmentsCompleted || 0}</h4>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-2xl font-bold">{stats?.averageScore || 0}%</h4>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-muted-foreground font-medium">Top Performers</p>
              <h4 className="text-2xl font-bold">{stats?.topPerformers || 0}</h4>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="overflow-hidden">
        <div className="p-6 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h3 className="text-xl font-display font-bold">Candidate Leaderboard</h3>
          <div className="flex items-center gap-2 bg-secondary p-1 rounded-xl">
            ...
            <select 
              ...
              value={filterType}
              onChange={(e) => setFilterType(e.target.value as GetCandidateRankingsAssessmentType)}
            >
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              {rankings?.map((candidate) => (
                <tr key={candidate.candidateId} className="hover:bg-secondary/20 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-semibold text-foreground">{candidate.targetRole}</div>
                    <div className="text-muted-foreground text-xs">{candidate.experienceYears} Years Exp.</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex flex-col items-end gap-1">
                      ...
                      <div className="flex gap-1">
                        {candidate.codingScore != null && <span className="text-[10px] bg-blue-100 text-blue-700 px-1 rounded" title="Coding">C:{candidate.codingScore}</span>}
                        {candidate.aptitudeScore != null && <span className="text-[10px] bg-purple-100 text-purple-700 px-1 rounded" title="Aptitude">A:{candidate.aptitudeScore}</span>}
                        {candidate.technicalScore != null && <span className="text-[10px] bg-orange-100 text-orange-700 px-1 rounded" title="Technical">T:{candidate.technicalScore}</span>}
                      </div>
                    </div>
                  </td>
                </tr>
